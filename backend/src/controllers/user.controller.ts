...
const logOut = asyncHandler(async (req: Request, res: Response) => {
  ...
  return res
    ...
    .json(new ApiRes(200, "logged out", null));
});
...
const resetPassword = asyncHandler(async (req: Request, res: Response) => {
  ...
  const correctCurrentPass = await bcrypt.compare(
    currentPassword,
    user?.password,
  );
  ...
  return res
    .status(201)
    .json(new ApiRes(201, "password updated successfully", null));
});
...
const refreshAccessToken = asyncHandler(async (req: Request, res: Response) => {
  ...
  const newRefreshToken = jwt.sign({ id: user._id }, config.rt_jwt, {
    expiresIn: "25d",
  });
  ...
  return res
    ...
    .json(
      new ApiRes(201, "new access_token and refresh_token generated", {
        newAccess_token,
      }),
    );
});

export { signIn, signUp, logOut, resetPassword, refreshAccessToken };
